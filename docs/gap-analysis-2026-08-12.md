...
**⑨ allow/deny 规则可视化配置 UI（agent-design P0-2 收尾）**
- 现状：权限三档已有；但"命令级 + 文件级 allow/deny 规则"的可视化配置（类 Cursor Add to allowlist、Claude Code settings.json allow/deny）未做。
- 价值：中。
...
> 更新（2026-08-12）：原 P0 中 ② 前端附件、③ Plan Mode、④ 身份复验经代码复核均为**已落地**（②③为新实现/确认，④为确认）。⑦ @提及注入亦确认已落地。**① MCP 已于 2026-08-12 实现并验证**（commit `0a448f3`），故 P0 全部清零。**⑥ 实时增量代码索引已于 2026-08-12 实现并验证**（commit `4c831f4`）。剩余未做项见下。

1. ~~**① MCP（唯一剩余 P0）**：决定生态扩展性，工作量较大，单独排期。**→ 已完成。**~~
2. ~~**⑥ 实时增量代码索引（P1）**：文件落盘即刷新语义索引，模型新写代码可立即检索。**→ 已完成。**~~
3. **P1 按 ROI 挑 ⑤ fast-apply、⑨ allow/deny UI**（直接改善体感与可靠性），⑧ /undo 可后置。
4. **P2 全部按需**（⑩⑪⑫⑬），属"锦上添花"。

> 说明：本报告最初为纯交叉核对（未改代码）；2026-08-12 已据此实现并验证 **③ 真实 Plan Mode 硬开关**、**① MCP 外部工具接入**、**⑥ 实时增量代码索引** 三项。P0 缺口已全部清零，P1 已落地 1 项（⑥）。
